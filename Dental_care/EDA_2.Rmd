...
plotChart <- function(name, sheetIndex) {
 ...
 p <-  ggplot(DT[Country != "Switzerland"], aes(Country, `2008`)) +
    ...
    labs(title = name, subtitle = "", caption = "") +
    coord_flip() +
...
```{r}
plotChart("Dental expenditure AS % OF TOTAL HEALTH EXPENDITURE", 1)
plotChart("Public as % of total dental care expenditure", 2)
plotChart("Dataset: Health expenditure and financing", 3)
plotChart("Private as % of total", 4)
plotChart("Self-reported unmet needs for dental examination by sex, age, main reason declared and income quintile [hlth_silc_09]", 5)
```
